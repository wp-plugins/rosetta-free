 ...
 */
if( ! class_exists('Theme_Menus_Localizer') ){
	...
	function admin_enqueue( $hook ) {
		...
		if ( is_nav_menu( $menu ) ) {
			...
			foreach( $menu_items as $item ) {
				foreach ( $this->langs['others'] as $prefix => $lang ) {
					$title_new = get_post_meta( $item->ID, 'title-' . $lang['prefix'], true );
		...
		wp_enqueue_script( 'menu-localizer', Utilities::plugins_url() . 'js/menu-localizer.js' );
	...
	function update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array() ) {
		...
		foreach ( $this->langs['others'] as $prefix => $lang ) {
			$item_meta['title-' . $lang['prefix']] = $_POST['rosetta-menu-item-title'][$lang['prefix']][$menu_item_db_id];
		}
	...
	$theme_menu_localizer = new Theme_Menus_Localizer();
}
