 ...
 */
if( ! class_exists('Bloginfo_Localizer') ){
	...
	function localize_blogname($value){
		...
		if (CURRENT_LANG != DEFAULT_LANG){
			...
			$value_new = ( is_multisite() ) ? get_blog_option( get_current_blog_id(), 'blogname-'.CURRENT_LANG ) : get_option( 'blogname-'.CURRENT_LANG );
	...
	function localize_blogdescription($value){
		...
		if (CURRENT_LANG != DEFAULT_LANG){
			...
			$value_new =( is_multisite() ) ? get_blog_option( get_current_blog_id(), 'blogdescription-'.CURRENT_LANG ) : get_option( 'blogdescription-'.CURRENT_LANG );
	...
	function add_scripts() {
		...
		wp_enqueue_script('rosetta_bloginfo_tabs', Utilities::plugins_url() . 'js/bloginfo-tabs.js');
		
		( false == strpos($_SERVER['REQUEST_URI'],"site-settings.php") ) ? $mu_settings_page = false : $mu_settings_page = true;
		
		wp_localize_script('rosetta_bloginfo_tabs', 'bloginfo_langs', json_encode(array('langs' => $this->get_indexed_language_list(), 'default_str' => __('Default','rosetta-plugin'), 'mu_settings_page' => $mu_settings_page)));
	}
